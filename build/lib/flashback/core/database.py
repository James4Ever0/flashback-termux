...
class Database:
    ...
    def get_app_name_from_history(self, app_id: str) -> Optional[str]:
        """Get human-readable app name from historical screenshots.

        Looks for window titles associated with the given app_id.
        Returns the most common non-package-name window title.

        Args:
            app_id: Package ID (e.g., "com.android.chrome")

        Returns:
            Human-readable app name or None if not found
        """
        with self._connect() as conn:
            # Look for window titles that are not the package name itself
            # and not "Unknown" - these are likely human-readable names
            cursor = conn.execute(
                """
                ...
                AND window_title NOT LIKE '%/%'
                AND (
                    -- Window title contains app_id substring (e.g., "Chrome" in "com.android.chrome")
                    -- or we previously resolved this app
                    EXISTS (
                        SELECT 1 FROM screenshots s2
                        WHERE s2.window_title = ?
                        LIMIT 1
                    )
                ...
                LIMIT 1
                """,
                (app_id, app_id, f"%{app_id.split('.')[-1].replace('_', ' ')}%")
            )
            row = cursor.fetchone()
            if row and row["window_title"]:
                return row["window_title"]

            # Fallback: just get any non-null, non-package window title
            ...
            # Return the first result that looks like a proper app name
            for row in results:
                title = row["window_title"]
                if title and len(title) > 1 and not title.startswith('com.'):
                    return title
