...
class WindowTitleWorker(IntervalWorker):
    ...
    def _init_resources(self):
        ...
        # Options: "focused" (current focused display), "all" (track all), or specific number (e.g., 0)
        target_display_config = self.config.get("workers.window_title.target_display", "focused")
        if target_display_config == "focused":
            self._target_display = None  # Track focused display
        elif target_display_config == "all":
            self._target_display = -1  # Track all displays
        else:
            try:
                self._target_display = int(target_display_config)
            except (ValueError, TypeError):
                self._target_display = None  # Default to focused
